...
void DrawChunk::ChunkDataToMesh(glm::vec2 chunkCoord, std::map<std::pair<int, int>, WorldChunkData*> worldDat)
{
	...
	//buffer data
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * chunkVertexBufferSize, chunkVertexBuffer, GL_STATIC_DRAW);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * chunkIndicieBufferSize, chunkIndicieBuffer, GL_STATIC_DRAW);
